...
class Drawer:
    ...
    def _load_data(self):
        ...
        self.hit_channel_count = len(target_pmt_id) if isinstance(target_pmt_id, np.ndarray) else len(self.pmt_channels)
    ...
    def draw_vertices_density(self, figure, axis):
    ...
    def draw_pe_hit_time(self, figure, axis):
    ...
    def draw_probe(self, figure, axis):
        ...
        r_corr = r_centers * r_centers
        theta_corr = np.sin(theta_centers)
        rho = len(self.vertices) / (4 / 3 * np.pi)
        # Create correction matrix
        correction_matrix = 2 * (np.pi ** 2) * rho / 2500 * np.outer(r_corr, theta_corr)
        # Avoid division by zero
        correction_matrix[correction_matrix == 0] = 1e-9

        # Apply correction.
        probe_function = H_avg / correction_matrix # Transpose to match H_avg shape
...
def main():
    ...
    try:
        probe = Drawer(args.input_file, args.geometry)
        ...
        with PdfPages(args.output) as pdf:
            ...
            probe.draw_vertices_density(fig1, ax1)
            ...
            probe.draw_pe_hit_time(fig2, ax2)
